"""
BookingScraper/app/extractor.py  v2.5
...
CAMBIOS v2.5:
  [FIX CRITICO] extract_address() JSON-LD: address duplicada eliminada.
    streetAddress ya contiene city y region; addressLocality y addressRegion
    se anadiran SOLO si no estan ya en streetAddress (comparacion lowercase).
    Antes: 'N Coast Road, Mahe, Beau Vallon, Seychelles, N Coast Road, Mahe, Seychelles'
    Ahora: 'N Coast Road, Mahe, Beau Vallon, Seychelles'
"""
...
class BookingExtractor:
    ...
    def extract_name(self) -> Optional[str]:
        ...
        og:title es el MÁS FIABLE porque siempre está en el SSR.
        """
        # 1. og:title (primera opción — siempre presente en páginas reales)
        v = self._meta(prop="og:title")
        if v:
            # Booking.com a veces añade " | Booking.com" al final
            v = re.sub(r'\s*[|\-–]\s*Booking\.com\s*$', '', v, flags=re.IGNORECASE).strip()
            if v:
                logger.debug(f"  Nombre extraído vía: og:title")
                return v
        ...
        if v:
            v = re.sub(r'\s*[|\-–]\s*Booking\.com\s*$', '', v, flags=re.IGNORECASE).strip()
            if v:
                logger.debug(f"  Nombre extraído vía: meta[name=title]")
                return v

        # 3. data-testid="title" (estructura 2024-2025)
        if self.soup:
    ...
    def extract_rating_category(self) -> Optional[str]:
        ...
        CATEGORIES = {
            "en": ["Exceptional", "Superb", "Fabulous", "Very good", "Good", "Pleasant", "No rating"],
            "es": ["Excepcional", "Fabuloso", "Espléndido", "Muy bien", "Bien", "Agradable"],
            "de": ["Hervorragend", "Fantastisch", "Ausgezeichnet", "Sehr gut", "Gut"],
            "fr": ["Exceptionnel", "Fabuleux", "Superbe", "Très bien", "Bien"],
            "it": ["Eccezionale", "Favoloso", "Fantastico", "Molto buono", "Buono"],
        }
        search_cats = (
        ...
        if self.soup:
            # Buscar en review-score component
            elem = self.soup.find(attrs={"data-testid": "review-score-component"})
            if elem:
                text = elem.get_text()
    ...
    def extract_images(self) -> List[str]:
        """
        [v2.3] Solo extrae FOTOS REALES del hotel.

        CDN real de Booking.com:  cf.bstatic.com/xdata/images/hotel/
        Ruido descartado:
          - t-cf.bstatic.com/design-assets/  (banderas de paises, iconos UI)
          - xx.bstatic.com/static/img/review/ (avatares de resenadores)
          - r-xx.bstatic.com/images/user/     (fotos de perfil de usuarios)
          - *.gif, 1x1 pixels, tracking pixels
        """
        images = []
        seen   = set()
