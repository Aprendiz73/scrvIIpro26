"""
BookingScraper/app/scraper_service.py  v2.3
        ...
        try:
            for lang in languages:
                ...
                try:
                    ...
                    # Imágenes (solo idioma base)
                    if lang == settings.DEFAULT_LANGUAGE and settings.DOWNLOAD_IMAGES:
                        imgs = data.get("images_urls") or []
                        if imgs:
                            # Pasar el driver Brave para usar sus cookies (evita bloqueo CDN)
                            driver = scraper_instance.driver if settings.USE_SELENIUM else None
                            _download_images(url_id, imgs[:30], lang, driver=driver)
