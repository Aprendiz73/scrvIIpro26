...
def _get_redis() -> "Optional[_redis_lib.Redis]":
    """
    [FIX ERR-ARCH-002] Return a healthy Redis client, or None.
    On each call, performs a lightweight health-check; if Redis is down,
    attempts to reconnect once before falling back to local _active_ids.
    This prevents silent failures after an initial successful connection.
    """
    global _redis_client, _redis_pool
    if _redis_client is None:
        return None
    try:
        _redis_client.ping()  # fast O(1) check
        return _redis_client
    except Exception as hc_err:
        logger.warning("[ARCH-002] Redis health-check falló (%s) — intentando reconexión...", hc_err)
        try:
            _redis_client = _redis_lib.Redis(connection_pool=_redis_pool)
            _redis_client.ping()
            logger.info("[ARCH-002] Redis reconectado exitosamente.")
            return _redis_client
        except Exception as reconn_err:
            logger.error("[ARCH-002] Redis reconexión falló (%s) — fallback a memoria.", reconn_err)
            _redis_client = None
            return None
...
def _claim_active_url(url_id: int) -> bool:
    ...
    if r is not None:
        try:
            ...
            logger.warning("[ARCH-002] Redis claim error (fallback local): %s", e)
