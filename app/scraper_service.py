...
CORRECCIONES v2.0 [NEW-03, NEW-08, NEW-12, NEW-14]: ver inline [FIX ...] comments.
"""
...
# Pool de threads
# [NEW-12] Longitudes máximas de mensajes de error — estandarizadas en todo el módulo
_ERR_TRUNCATE_SHORT: int = 200   # para logs de nivel ERROR en pantalla
_ERR_TRUNCATE_DB:    int = 500   # para almacenamiento en scraping_logs.error_message
            ...
            for lang in languages:
                ...
                for attempt in range(1, MAX_LANG_RETRIES + 2):
                    try:
                        ...
                        logger.error(f"  [{url_id}][{lang}] attempt={attempt}: {err_str[:_ERR_TRUNCATE_SHORT]}")
                        try:
                            db.rollback()
                        except Exception as _rb_err:
                            # [NEW-08] Rollback failures logged — not silently swallowed
                            logger.debug(f"  [{url_id}] rollback warning: {_rb_err}")
                        ...
                        if settings.USE_SELENIUM and "invalid session id" in err_str.lower():
                            ...
                            try:
                                ...
                                # [NEW-08] Browser close failures logged
                                logger.debug(f"  [{url_id}] browser close warning: {_close_err}")
                            try:
                                scraper_instance = BookingScraper()
                                data = scraper_instance.scrape_hotel(lang_url, language=lang)
                                        ...
                                        if imgs:
                                            ...
                                            if n_dl and n_dl > 0:
                                                try:
                                                    ...
                                                    db.commit()
                                                except Exception as _upd2_err:
                                                    # [NEW-08] images_count update failure logged
                                                    logger.warning(f"  [{url_id}] images_count update failed: {_upd2_err}")
                                        images_downloaded = True
                                    lang_succeeded = True
                                    break
                        ...
                        _log(db, url_id, lang, "error", time.time() - start_time, 0, err_str[:_ERR_TRUNCATE_DB])
                        has_retry = completeness_service.record_language_failure(
...
# [NEW-03] Constantes para sanitización de valores JSON
_MAX_STR_FIELD_LEN: int  = 50_000  # caracteres máximos por campo de texto largo
_MAX_URL_ITEM_LEN:  int  = 2_048   # longitud máxima por URL individual en listas JSON


def _sanitize_json_value(value) -> object:
    """
    [NEW-03] Sanitiza un valor antes de serializarlo a JSON para inserción en BD.
    Previene inyección de contenido XSS/HTML en campos JSONB que serán
    consumidos por frontends u otras aplicaciones.
    Trunca strings excesivamente largos que podrían indicar datos maliciosos.
    """
    if isinstance(value, str):
        # Truncar strings anormalmente largos
        return value[:_MAX_STR_FIELD_LEN]
    if isinstance(value, list):
        sanitized = []
        for item in value:
            if isinstance(item, str):
                sanitized.append(item[:_MAX_URL_ITEM_LEN])
            elif isinstance(item, dict):
                sanitized.append({k: _sanitize_json_value(v) for k, v in item.items()})
            else:
...
def _save_hotel(db, url_id: int, url: str, lang: str, data: Dict) -> bool:
    ...
    [NEW-03] Todos los valores JSON pasan por _sanitize_json_value() antes de
    json.dumps() para prevenir stored XSS y datos malformados en JSONB.
    """
    result = db.execute(
        text("""
            INSERT INTO hotels (
        ...
        """),
        {
            ...
            # [NEW-03] _sanitize_json_value() antes de json.dumps — previene XSS/injection
            "review_scores":    json.dumps(_sanitize_json_value(data.get("review_scores") or {})),
            "services":         json.dumps(_sanitize_json_value(data.get("services")      or [])),
            "facilities":       json.dumps(_sanitize_json_value(data.get("facilities")    or {})),
            "house_rules":      (data.get("house_rules")     or "")[:_MAX_STR_FIELD_LEN] or None,
            "important_info":   (data.get("important_info")  or "")[:_MAX_STR_FIELD_LEN] or None,
            "rooms_info":       json.dumps(_sanitize_json_value(data.get("rooms")         or [])),
            "images_urls":      json.dumps(_sanitize_json_value(data.get("images_urls")   or [])),
        }
    )
    db.commit()
...
def _log(db, url_id: int, language: str, status: str,
         duration: float, items: int, error: Optional[str] = None) -> None:  # [NEW-14]
