...
try:
    ...
    if _REDIS_URL:
        _redis_client = _redis_lib.Redis.from_url(_REDIS_URL, socket_connect_timeout=2)
        _redis_client.ping()
        logger.info("[ARCH-002] Redis conectado — _active_ids compartido entre workers")
    else:
        _redis_client = None
        logger.warning(
            "[ARCH-002] REDIS_URL no configurado — _active_ids solo en memoria "
            "(single-process). Configure REDIS_URL para escalar horizontalmente."
        )
except Exception as _redis_init_err:
    _redis_client = None
    logger.warning(
        "[ARCH-002] Redis no disponible (%s) — fallback a memoria (single-process).",
