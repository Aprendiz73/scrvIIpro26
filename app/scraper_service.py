...
# (SIGKILL excepted). This prevents OS-level thread leaks on repeated restarts.
...
try:
    ...
    # [FIX ARCH-002] Usar settings.REDIS_URL para consistencia con config.py.
    # settings.REDIS_URL se construye desde REDIS_HOST/PORT/DB/PASSWORD del .env,
    # igual que el broker de Celery (Memurai en Windows).
    # En Windows local (SCRAPER_MAX_WORKERS=1), el modo single-process con
    # fallback local es el COMPORTAMIENTO NORMAL — Redis es opcional.
    _redis_client = _redis_lib.Redis.from_url(settings.REDIS_URL, socket_connect_timeout=2)
    _redis_client.ping()
    logger.info(
        "[ARCH-002] Redis/Memurai conectado (%s) — _active_ids compartido entre workers",
        _REDIS_URL.split("@")[-1],  # no exponer password en log
    )
except Exception as _redis_init_err:
    _redis_client = None
    logger.info(
        "[ARCH-002] Redis/Memurai no disponible (%s) — usando _active_ids local "
        "(modo normal en Windows local con SCRAPER_MAX_WORKERS=1).",
        type(_redis_init_err).__name__,
    )
...
def _claim_active_url(url_id: int) -> bool:
    ...
    Retorna True si el claim fue exitoso (no estaba activo en ningún worker).
    Retorna False si ya estaba activo (en este proceso u otro vía Redis).
    """
    if _redis_client is not None:
        try:
            ...
            logger.warning("[ARCH-002] Redis claim error, usando fallback local: %s", e)
    # Fallback local
...
def _release_active_url(url_id: int) -> None:
    ...
    if _redis_client is not None:
        try:
            _redis_client.delete(f"bsp:active:{url_id}")
            return
        except Exception as e:
            logger.warning("[ARCH-002] Redis release error: %s", e)
    with _active_ids_local_lock:
    ...
    with _vpn_lock_ctx():
        try:
            ...
            if success:
                ...
                # Without this, vpn.rotate() returning False left the counter elevated,
                # triggering another rotation attempt on the very next hotel — rapid
                # reconnection that can rate-limit the VPN provider.
                # Reset to 1 (not 0): acknowledges the failure without re-triggering
                # immediately. Next genuine failure re-evaluates normally.
                with _stats_lock:
...
def process_batch(batch_size: int = 5) -> Dict:
    ...
    db = SessionLocal()
    try:
        # [FIX BUG-NEW-001] Atomic dispatch: SELECT ... FOR UPDATE SKIP LOCKED.
        # Previous approach (SELECT then UPDATE in separate statements) had a
        # race condition: two concurrent workers could SELECT the same URL IDs
        # before either issued the UPDATE, causing double-dispatch.
        #
        # FOR UPDATE SKIP LOCKED acquires a row-level lock atomically during the
        # SELECT. Any row already locked by another transaction (another worker)
        # is silently skipped — never selected. The UPDATE runs inside the same
        # transaction, holding the lock. This guarantees each URL is dispatched
        # exactly once even with concurrent callers (horizontal scaling).
        #
        # The in-process _active_ids guard is kept as a secondary defense for
        # same-process concurrent asyncio tasks, but FOR UPDATE SKIP LOCKED is
        # the primary correctness guarantee.
        rows = db.execute(
            text("""
        ...
        # [FIX ARCH-002] Claim URLs vía Redis atómico (o fallback local).
        # _claim_active_url retorna False si el url_id ya está activo en cualquier worker.
...
def _log(db, url_id: int, language: str, status: str,
    ...
    # [FIX BUG-NEW-11] Truncate error_message to prevent table bloat.
    # Full HTML responses in errors can be megabytes; 2000 chars is sufficient for diagnosis.
    _MAX_ERROR_LEN = 2000
    if error and len(error) > _MAX_ERROR_LEN:
        error = error[:_MAX_ERROR_LEN] + f"... [truncated {len(error) - _MAX_ERROR_LEN} chars]"
    try:
        db.execute(
            text("""
