...
-- [FIX BUG-V5-005] Índice dedicado sobre hotels.url_id (FK → url_queue.id).
-- Sin este índice, los JOIN y CASCADE DELETE hacen Seq Scan completo de hotels.
-- Con 100 k filas: JOIN 100ms→10s, CASCADE DELETE 10ms→5s (degradación x100).
CREATE INDEX IF NOT EXISTS idx_hotels_url_id
    ON hotels (url_id);
...
CREATE TABLE IF NOT EXISTS url_language_status (
    ...
    -- [FIX BUG-V5-009] 'skipped_existing' añadido — completeness_service.py
    -- usa este valor en record_language_skipped(). Sin él, el INSERT/UPDATE
    -- fallaba con constraint violation silenciosamente.
    CONSTRAINT uls_status_check CHECK (
        status IN ('pending','processing','completed','failed','skipped','skipped_existing')
    )
);
