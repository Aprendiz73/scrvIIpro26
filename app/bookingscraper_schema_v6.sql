...
BEGIN
    ...
    -- [FIX BUG-V13-001] Expand status constraint to include 'incomplete'.
    -- 'incomplete' = URL processed but some languages failed (partial success).
    -- DROP+ADD pattern required — PostgreSQL does not support ALTER CONSTRAINT for CHECK.
    IF EXISTS (
        SELECT 1 FROM pg_constraint
    ...
    -- [FIX BUG-V13-002] Expand status constraint to include 'skipped_existing'.
    -- 'skipped_existing' = hotel data already in DB; idempotent skip counted as success.
    -- DROP+ADD pattern required — PostgreSQL does not support ALTER CONSTRAINT for CHECK.
    IF EXISTS (
        SELECT 1 FROM pg_constraint
