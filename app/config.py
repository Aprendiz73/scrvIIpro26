...
class Settings(BaseSettings):
    ...
    APP_VERSION: str = "6.0.0"           # [FIX BUG-NEW-04] alineado con versión real del sistema
    DEBUG: bool = False
    ...
    def _validate_settings(self) -> "Settings":
        ...
        # BUG-V5-021: Validar cada idioma como código ISO 639-1 (exactamente 2 letras a-z)
        _VALID_ISO_639_1 = {
        ...
        enabled = [lang.strip() for lang in self.LANGUAGES_ENABLED.split(",") if lang.strip()]
        for lang in enabled:
            if lang not in _VALID_ISO_639_1:
                raise ValueError(
                    f"LANGUAGES_ENABLED contiene código inválido: '{lang}'. "
                    f"Usar solo códigos ISO 639-1 de 2 letras (ej: en, es, de, fr, it)."
                )
        return self
    ...
    # ── RUTAS ─────────────────────────────────────────────────────────────────
    # [NEW-15] os.path.join para portabilidad cross-platform (Linux/Docker/Windows).
    # Sobreescribir en .env: BASE_DATA_PATH=/data  IMAGES_PATH=/data/images  etc.
