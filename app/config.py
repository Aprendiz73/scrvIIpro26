...
CORRECCIONES v2.0 [NEW-01, NEW-15, NEW-16]: ver inline [FIX ...] comments.
"""
...
class Settings(BaseSettings):
    ...
    # [FIX NEW-01] DB_PASSWORD: credencial eliminada del código fuente.
    # NUNCA hardcodear contraseñas en código versionado.
    # Configurar obligatoriamente en .env o variable de entorno:
    #   DB_PASSWORD=tu_contraseña_segura
    # El valor "2221" se mantiene como fallback de desarrollo LOCAL ÚNICAMENTE.
    # En producción/CI/Docker: asegurarse de que DB_PASSWORD esté en el entorno.
    DB_PASSWORD: str = "change_me_in_dotenv"
    ...
    # ── RUTAS ─────────────────────────────────────────────────────
    # [NEW-15] Rutas cross-platform configurables via .env.
    # Valores por defecto: Windows (C:\BookingScraper\data) si no se sobreescriben.
    # En Linux/macOS: definir BASE_DATA_PATH=/home/user/bookingscraper en .env
    # Docker: montar volumen y definir BASE_DATA_PATH=/data
    BASE_DATA_PATH: str = os.path.join("C:\\BookingScraper", "data")
    IMAGES_PATH:    str = os.path.join("C:\\BookingScraper", "data", "images")
    EXPORTS_PATH:   str = os.path.join("C:\\BookingScraper", "data", "exports")
    LOGS_PATH:      str = os.path.join("C:\\BookingScraper", "data", "logs")

    # ── RUTAS DE NAVEGADORES ─────────────────────────────────────
    # [NEW-16] Rutas de ejecutables de navegador configurables via .env.
    # Antes estaban hardcodeadas en scraper.py con path de usuario específico (SA).
    # Definir en .env para sobreescribir según el entorno:
    #   BROWSER_BRAVE_PATH=C:\Program Files\BraveSoftware\Brave-Browser\Application\brave.exe
    #   BROWSER_OPERA_PATH=C:\Users\MiUsuario\AppData\Local\Programs\Opera\opera.exe
    BROWSER_BRAVE_PATH:  str = ""   # vacío = usar lista de rutas predeterminadas
