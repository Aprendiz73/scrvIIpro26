...
CORRECCIONES v1.3 [BUG FIXES]:
  [FIX BUG-06] create_directories() eliminada del nivel de módulo.
    PROBLEMA: Se ejecutaba automáticamente en cada import, causando
    PermissionError si el proceso no tenía acceso a C:\\BookingScraper\\.
    SOLUCIÓN: Moverla al lifespan de FastAPI con manejo de excepción.
  [FIX BUG-11] USE_CELERY_DISPATCHER: bool añadida.
    PROBLEMA: auto-dispatcher asyncio (main.py) y Celery beat operaban
    simultáneamente sin coordinación, procesando el doble de URLs esperadas.
    SOLUCIÓN: Flag para activar solo uno de los dos mecanismos.
"""
...
class Settings(BaseSettings):
    ...
    # ── DISPATCHER ───────────────────────────────────────────────
    # [FIX BUG-11] Controla cuál mecanismo de dispatch está activo.
    # False (default) = asyncio auto-dispatcher en main.py (sin Celery).
    # True = Celery beat. NUNCA activar ambos simultáneamente:
    # producen double-dispatch sin coordinación entre procesos.
...
def create_directories() -> None:
    ...
    [FIX BUG-06] NO llamar desde nivel de módulo: moverla al lifespan
    de FastAPI con manejo de excepción para no bloquear el arranque.
    """
    for path in [
...
# [FIX BUG-06] ELIMINADO: create_directories() ya NO se llama aquí.
# Se llama desde el lifespan de FastAPI (main.py) con try/except,
# de modo que un PermissionError no impide el arranque del servidor.
