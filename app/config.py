...
class Settings(BaseSettings):
    ...
    # ── RUTAS ─────────────────────────────────────────────────────────────────
    # [FIX BUG-A-11] Default paths were hardcoded to C:\\BookingScraper, making
    # the app non-functional on any non-Windows system (Linux, Docker, macOS)
    # without setting env vars. Default is now relative './data', which works
    # on all platforms. Production deployments override via .env as before.
    # Sobreescribir en .env:
    #   Windows: BASE_DATA_PATH=C:\\BookingScraper\\data
    #   Linux:   BASE_DATA_PATH=/opt/bookingscraper/data
    BASE_DATA_PATH: str = os.path.join(".", "data")
    IMAGES_PATH:    str = os.path.join(".", "data", "images")
    EXPORTS_PATH:   str = os.path.join(".", "data", "exports")
    LOGS_PATH:      str = os.path.join(".", "data", "logs")
