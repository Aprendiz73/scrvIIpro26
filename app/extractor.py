...
CORRECCIONES v2.0 [NEW-11]: ver inline [FIX ...] comments.
"""
...
class BookingExtractor:
    ...
    def _validate_lang(self, text: str, language: str = None) -> bool:
        ...
        # [NEW-11] Named constants replacing magic numbers 30 and 3
        _MIN_TEXT_LEN_FOR_LANG_CHECK = 30   # chars; shorter texts → indeterminate language
        _NEG_SIGNAL_THRESHOLD        = 3    # minimum neg signals to reject language
        if not text or len(text.strip()) < _MIN_TEXT_LEN_FOR_LANG_CHECK:
            return True
        ...
        if score_neg >= _NEG_SIGNAL_THRESHOLD and score_neg > score_pos:
            logger.debug(
