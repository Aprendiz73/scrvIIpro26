...
class ImageDownloader:
    ...
    def get_local_paths(hotel_id: int, lang: str = "en") -> List[str]:
        """
        [FIX BUG-03] Devuelve la lista de rutas locales de imágenes descargadas
        para el hotel dado. Necesario para actualizar hotels.images_local en BD.

        Returns:
            Lista de rutas absolutas (str) de archivos de imagen existentes.
            Lista vacía si el directorio no existe o no hay archivos.
        """
        try:
            ...
            return [
                str(f) for f in sorted(base.rglob("*"))
                if f.is_file() and f.suffix.lower() in extensions
            ]
        except Exception as e:
            logger.debug(f"get_local_paths({hotel_id}, {lang}): {e}")
            return []
