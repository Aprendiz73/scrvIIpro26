...
CORRECCIONES v2.0 [NEW-06]: ver inline [FIX ...] comments.
"""
...
class ImageDownloader:
    ...
    def download_images(
        ...
        # [NEW-06] ThreadPoolExecutor con timeout explícito por imagen.
        # El bloque 'with' garantiza shutdown(wait=True) al salir,
        # liberando recursos de forma determinista sin depender del GC.
        # timeout=90s por future.result() previene bloqueos indefinidos.
        _SINGLE_IMG_TIMEOUT_S = 90  # segundos máximo por imagen individual
        with ThreadPoolExecutor(
            max_workers=self.max_workers,
            thread_name_prefix="imgdl",
        ) as executor:
            futures = {
                executor.submit(self._download_single, url, target_dir, idx, hotel_id, room_id): url
                for idx, url in enumerate(image_urls)
            }
            for future in as_completed(futures, timeout=_SINGLE_IMG_TIMEOUT_S * len(image_urls)):
                url = futures[future]
                try:
                    result = future.result(timeout=_SINGLE_IMG_TIMEOUT_S)
                    if result:
                    ...
                    logger.warning(f"Timeout descargando {url[:80]}")
                    self.stats["failed"] += 1
