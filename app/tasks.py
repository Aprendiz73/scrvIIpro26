...
def cleanup_old_logs(self, days_to_keep: int = 30) -> Dict[str, Any]:
    ...
    try:
        result = db.execute(
            ...
            ).bindparams(days=days_to_keep)
        )
        # Alternativa compatible con parámetros posicionales si bindparams falla:
        # result = db.execute(
        #     sa_text(f"DELETE FROM scraping_logs WHERE timestamp < NOW() - INTERVAL '{days_to_keep} days'")
        # )
    ...
    try:
        db.execute(
            sa_text(
                ...
                "(cpu_percent, ram_percent, disk_percent, recorded_at) "
