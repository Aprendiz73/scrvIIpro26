...
CORRECCIONES v1.2 [FIX BUG-02]:
  [FIX BUG-02] Eliminado Base = declarative_base() propio.
    PROBLEMA: database.py y models.py tenían instancias separadas de Base,
    con MetaData distintos. Base.metadata.create_all(engine) en database.py
    no creaba ninguna tabla de models.py. Error silencioso.
    SOLUCIÓN: importar Base desde models.py (fuente única de verdad).
"""
...
# [FIX BUG-02] Base importada desde models.py — fuente única de verdad.
# ANTES: Base = declarative_base()  ← instancia vacía, incompatible con models.py
# AHORA: la misma instancia de Base que define todas las tablas reales.
from app.models import Base  # noqa: F401 — re-exportado para compatibilidad
