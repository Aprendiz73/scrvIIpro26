...
# [FIX BUG-NEW-18] No hardcoded default password. If DB_PASSWORD is not set,
# we raise explicitly at startup rather than silently using a placeholder credential.
...
# ── MOTOR ──────────────────────────────────────────────────────────────────────
# [FIX BUG-16-005] Pool configuration now reads from environment variables.
# Previously hardcoded (pool_size=5, max_overflow=10) — ignored DB_POOL_SIZE and
# DB_MAX_OVERFLOW from .env entirely. Operators had no way to tune pool sizing
# for production without editing source code.
...
engine = create_engine(
    ...
    pool_recycle=_POOL_RECYCLE,  # Recicla conexiones (default: 30 min)
    # [FIX BUG-V5-004] Timeouts para evitar bloqueo indefinido bajo carga.
    # pool_timeout: tiempo máximo esperando una conexión libre del pool.
...
def get_db():
    ...
    [FIX BUG-NEW-15] pool_pre_ping handles most stale connections, but first-query
    failures can still occur after PostgreSQL restarts. The caller should wrap
    critical operations in execute_with_retry() for resilience.
    """
    db = SessionLocal()
    try:
        yield db
...
def execute_with_retry(session_factory, operation, max_retries: int = 3, backoff: float = 0.5):
    """
    [FIX BUG-NEW-15] Execute a database operation with automatic retry on transient
    connection errors (e.g., first query on stale connection after PG restart).
    
    Usage:
        result = execute_with_retry(SessionLocal, lambda db: db.execute(text("SELECT 1")))
    """
    import time as _t
    # [FIX BUG-V7-007] Added InterfaceError — raised when PG server restarts or network
    # is interrupted mid-query. Previously these errors were NOT retried, causing
    # permanent failure for operations that could succeed on a fresh connection.
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            last_exc = e
            logger.warning(
                f"[BUG-NEW-15] DB transient error (attempt {attempt + 1}/{max_retries}): {e}"
            )
