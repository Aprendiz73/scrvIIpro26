...
CORRECCIONES v2.0 [NEW-01, NEW-07]: ver inline [FIX ...] comments.
"""
...
# [FIX NEW-01] Default de contraseña eliminado.
# DB_PASSWORD DEBE estar definido en el entorno o .env.
# Si no está definido, se usa "change_me_in_dotenv" como señal visible.
...
def get_pool_status() -> dict:
    """
    [NEW-07] Devuelve métricas de utilización del connection pool.
    Permite detectar saturación antes de que se manifieste como timeout.

    Returns:
        Dict con size, checked_out, overflow, checked_in, invalid
    """
    pool = engine.pool
    try:
        return {
            ...
            "utilization_pct": round(
                100.0 * pool.checkedout() / max(pool.size() + pool.overflow(), 1), 1
            ),
        }
...
def log_pool_status(threshold_pct: float = 80.0) -> None:
    """
    [NEW-07] Registra el estado del pool. Emite WARNING si utilización >= threshold_pct.
    Llamar desde endpoints de salud o periódicamente desde el dispatcher.
    """
    status = get_pool_status()
    pct = status.get("utilization_pct", 0)
    log_fn = logger.warning if pct >= threshold_pct else logger.debug
    log_fn(
        f"DB Pool | size={status.get('pool_size')} "
        f"checked_out={status.get('checked_out')} "
