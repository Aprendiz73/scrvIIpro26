...
class NordVPNManager:
    ...
    def rotate(self) -> bool:
        ...
        # [FIX BUG-08] Si 'available' está vacío pero hay países, reintentar cualquiera.
        # Si self.countries también está vacío → error claro en lugar de IndexError.
        if not available:
            available = list(self.countries)
        if not available:
            logger.error("✗ VPN_COUNTRIES está vacío — imposible rotar. "
                         "Verificar configuración VPN_COUNTRIES en .env")
            return False
