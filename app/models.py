...
from sqlalchemy import (
    ...
    Float, ForeignKey, Index, func, CheckConstraint
)
from sqlalchemy.orm import declarative_base, relationship
# [FIX BUG-NEW-002] Use PostgreSQL JSONB instead of generic JSON.
# JSONB: binary storage, GIN indexing, containment operators (@>, <@),
# better query performance. Ensures ORM (Base.metadata.create_all)
# creates JSONB columns matching bookingscraper_schema_v6.sql — closes
# the dev/prod column type parity gap introduced in Session 6.
...
class URLQueue(Base):
    ...
    __table_args__ = (
        ...
        # [FIX BUG-V13-005] ORM status constraint — mirrors bookingscraper_schema_v6.sql.
        # 'incomplete' = partial success (some languages failed after max retries).
        # Ensures dev (SQLAlchemy auto-create) and prod (SQL schema) enforce same values.
        CheckConstraint(
...
class Hotel(Base):
    ...
    review_scores       = Column(JSONB,    nullable=True)  # {"limpieza": 9.5, ...}

    # Servicios e instalaciones
    services    = Column(JSONB, nullable=True)   # ["WiFi", "Piscina", ...]
    facilities  = Column(JSONB, nullable=True)   # {"Categoría": ["item1", ...]}
    ...
    # Habitaciones
    rooms_info  = Column(JSONB, nullable=True)   # [{"name": ..., "description": ...}]

    # Imágenes
    images_urls  = Column(JSONB, nullable=True)  # ["https://...", ...]
    images_local = Column(JSONB, nullable=True)  # rutas locales descargadas
    ...
    # [FIX BUG-V7-003] Added explicit named Index for language-only queries.
    # While index=True on the Column creates an implicit index, the composite
    # ix_hotels_url_language (url_id, language) cannot satisfy WHERE language='en'
    # efficiently since 'language' is the non-leading column. An explicit single-column
    # index guarantees the query planner can use index-only scans for language filters.
    __table_args__ = (
        Index("ix_hotels_url_language", "url_id", "language", unique=True),
        Index("ix_hotels_language", "language"),   # [FIX BUG-V7-003] explicit language-only index
    )
...
class URLLanguageStatus(Base):
    ...
    __table_args__ = (
        ...
        # [FIX BUG-V13-005] ORM status constraint — mirrors bookingscraper_schema_v6.sql.
        # 'skipped_existing' = hotel data already present; idempotent skip = success.
        # Ensures dev (SQLAlchemy auto-create) and prod (SQL schema) enforce same values.
        CheckConstraint(
