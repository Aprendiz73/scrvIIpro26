...
CORRECCIONES v2.0 [NEW-04, NEW-05]: ver inline [FIX ...] comments.
"""
...
class Hotel(Base):
    ...
    __table_args__ = (
        ...
        # [NEW-05] GIN indexes for JSONB field queries (services, facilities)
        # Defined in init_db.sql as idx_hotels_services_gin / idx_hotels_facilities_gin
        # SQLAlchemy does not support GIN directly here; see init_db.sql for DDL.
    )
...
# =============================================================================
# [NEW-04/NEW-05] URLLanguageStatus — Tabla requerida por completeness_service.py
# Ausente en el init_db.sql original (divergencia documentada en NEW-04).
# Índice compuesto ix_uls_url_status añadido (NEW-05).
# =============================================================================

class URLLanguageStatus(Base):
    """Estado de scraping por URL + idioma. Una fila por combinación."""
    __tablename__ = "url_language_status"

    id          = Column(Integer, primary_key=True, autoincrement=True)
    url_id      = Column(Integer, ForeignKey("url_queue.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (
        ...
        # [NEW-05] Índice compuesto para consultas de retry eligibility
        # WHERE url_id = :id AND status IN ('pending', 'failed')
