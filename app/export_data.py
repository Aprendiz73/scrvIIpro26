...
from datetime import datetime, timezone  # [FIX BUG-A4] timezone-aware timestamps
...
# [FIX BUG-A1] SystemMetric → SystemMetrics (correct model class name in app/models.py)
from app.models import Hotel, URLQueue, ScrapingLog, VPNRotation, SystemMetrics
...
def export_hotels(session, format='csv', output_dir='data'):
    ...
    data = []
    for hotel in hotels:
        # [FIX BUG-A2] images_urls is a JSON list, not a CSV string.
        # Use len(list) if it's already parsed, or json.loads if stored as string.
        import json as _json
        _images = hotel.images_urls or []
        if isinstance(_images, str):
            try:
                _images = _json.loads(_images)
            except Exception:
                _images = []
        data.append({
            'id': hotel.id,
            ...
            'services': str(hotel.services)[:100] if hotel.services else None,
            'rooms_info': str(hotel.rooms_info)[:100] if hotel.rooms_info else None,
            'images_count': len(_images) if isinstance(_images, list) else 0,
            'created_at': hotel.scraped_at,
    ...
    # [FIX BUG-A-06] datetime.now(timezone.utc) — timezone-aware timestamp
    timestamp = datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')

    # [FIX BUG-B-01] Bloque de escritura de archivo faltaba por completo.
    # El código anterior creaba `timestamp` pero nunca definía `filename` ni
    # escribía el archivo, causando NameError en cada llamada a export_hotels().
    # Corregido: mismo patrón que export_urls(), export_logs() y export_statistics().
    if format == 'csv':
...
def export_statistics(session, format='csv', output_dir='data'):
    ...
    # [FIX BUG-A-04] Previous query was broken: `filter_by(language=Hotel.language)` passes
    # a Column object as the filter value, not a bound variable — SQLAlchemy evaluates it as
    # a tautology and the subquery always returns total hotel count regardless of language.
    # Correct approach: use func.count() with group_by() for a single efficient query.
    from sqlalchemy import func as _func
    languages = (
        session.query(Hotel.language, _func.count(Hotel.id).label("cnt"))
        .group_by(Hotel.language)
...
def main():
    ...
    try:
        ...
        # [FIX BUG-A-05] Always close session — previous code only called session.close()
        # inside the try block, leaking the connection on any exception path.
        try:
            session.close()
        except Exception:
            pass
