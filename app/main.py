...
async def lifespan(app: FastAPI):
    ...
    print("  BookingScraper Pro v6.0.0-r23 - Iniciando")
    print("=" * 60)
...
def search_hotels(q: str = Query(..., min_length=2, max_length=200), db: Session = Depends(get_db)):
    try:
        # [FIX C003] Escape LIKE wildcards (% and _) before building pattern.
        # Without escaping, a user query "50% discount" or "room_type" would expand
        # into unintended SQL wildcard patterns, returning broader results than expected.
        # Using PostgreSQL ESCAPE clause with backslash as escape character.
        q_safe = q.lower().replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
        rows = db.execute(text(
            "SELECT id, url_id, name, language, address, rating, total_reviews "
            "FROM hotels WHERE LOWER(name) LIKE :q ESCAPE '\\' ORDER BY name LIMIT 50"
        ), {"q": f"%{q_safe}%"}).fetchall()
