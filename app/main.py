...
async def load_urls_csv(
    ...
    **Limits:** 5 MB max file size, 5 000 URLs per upload.
    ...
    # Construir mapa url -> ext_id para el insert
    url_to_ext: Dict[str, Optional[str]] = {url: eid for eid, url in raw_pairs}
    ...
    with get_db() as session:
        for url in valid:
            ...
            existing = session.query(URLQueue).filter_by(url=url).first()
            if not existing:
                if has_ext and ext_id is not None:
                    session.execute(
                        text(
                            "INSERT INTO url_queue (url, base_url, external_ref) "
                            "VALUES (:url, :url, :ext_id) ON CONFLICT (url) DO NOTHING"
                        ),
