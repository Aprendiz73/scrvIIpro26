...
def _check_rate_limit(request: Request, limit: int = _RATE_LIMIT_MAX_REQUESTS) -> None:
    ...
    with _rate_lock:
        ...
        # [FIX Section 6.5] Periodically remove empty/stale bucket entries.
        # Prevents unbounded memory growth from many unique client IPs.
        if now - _rate_last_cleanup > _RATE_CLEANUP_INTERVAL:
            stale_ips = [
                ip for ip, ts in _rate_buckets.items()
                if not ts or ts[-1] < now - _RATE_LIMIT_WINDOW
            ]
            for ip in stale_ips:
...
async def lifespan(app: FastAPI):
    ...
    print("\n" + "=" * 60)
    print("  BookingScraper Pro v7.0 - Iniciando")
    print("=" * 60)
    ...
    if "_watchdog_task" in dir() and not _watchdog_task.done():
    ...
    # [FIX BUG-NEW-09] Reset URLs 'processing' atascadas al apagar — evita bloqueos permanentes
    # si la app no reinicia (e.g., debugging, mantenimiento sin restart).
    try:
        from app.database import SessionLocal
