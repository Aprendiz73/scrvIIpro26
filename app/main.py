...
CORRECCIONES v2.0 [NEW-02, NEW-13, NEW-17]: ver inline [FIX ...] comments.
"""
...
from fastapi import FastAPI, BackgroundTasks, Body, Depends, File, Header, HTTPException, Query, Request, Security, UploadFile
...
# ─────────────────────────────────────────────────────────────────────────────
# [NEW-02] API AUTHENTICATION — API Key via X-API-Key header
#
# Configuracion en .env:
#   API_KEY=<clave_segura_minimo_32_chars>
#   API_ENABLED=True
#
# Rutas publicas (sin auth): GET / GET /health GET /docs GET /openapi.json
# Para generar clave:  python -c "import secrets; print(secrets.token_urlsafe(32))"
# ─────────────────────────────────────────────────────────────────────────────

_API_KEY: str       = os.getenv("API_KEY", "")
_API_ENABLED: bool  = os.getenv("API_ENABLED", "False").lower() == "true"


def _verify_api_key(x_api_key: Optional[str] = Header(default=None)) -> None:
    """[NEW-02] Verifica API Key con hmac.compare_digest (resistente a timing attacks)."""
    if not _API_ENABLED:
        return
    if not _API_KEY:
        logger.warning("API_ENABLED=True pero API_KEY no configurada")
        raise HTTPException(503, "API_KEY no configurada. Contactar al administrador.")
    if x_api_key is None:
        raise HTTPException(
            401,
            detail="API key requerida. Cabecera: X-API-Key: <clave>",
            headers={"WWW-Authenticate": "ApiKey"},
        )
    if not hmac.compare_digest(x_api_key.encode(), _API_KEY.encode()):
        logger.warning(f"Intento con API key invalida (len={len(x_api_key)})")
        raise HTTPException(403, "API key invalida o expirada.")
...
async def _auto_dispatch_loop():
    ...
    while True:
        try:
            # [NEW-13] asyncio.get_running_loop() — correcto para contexto async.
            # asyncio.get_event_loop() está deprecated en Python 3.10+ y genera
            # DeprecationWarning. get_running_loop() lanza RuntimeError si no hay
            # loop activo (imposible aquí dentro de 'async def'), siendo más seguro.
...
def health_check(db: Session = Depends(get_db)):
    ...
    # [NEW-07] Connection pool metrics
    from app.database import get_pool_status
    pool_info = get_pool_status()

    # [NEW-17] Redis health check — Celery tasks fail silently if Redis is down
    ...
    if settings.USE_CELERY_DISPATCHER:
        try:
            ...
            redis_status = f"error: {str(re)[:100]}"
...
def vpn_rotate(_: None = Depends(_verify_api_key)):
...
def vpn_connect(country: str = Body(default=None, embed=True), _: None = Depends(_verify_api_key)):
    """
...
@app.post("/urls/load", tags=["URLs"], dependencies=[Depends(_verify_api_key)])
...
@app.post("/urls/reset-failed", tags=["URLs"], dependencies=[Depends(_verify_api_key)])
...
@app.post("/urls/{url_id}/rollback", tags=["URLs"], dependencies=[Depends(_verify_api_key)])
def rollback_url(
    url_id: int,
...
@app.post("/scraping/start", tags=["Scraping"], dependencies=[Depends(_verify_api_key)])
def scraping_start(
...
@app.post("/scraping/force-now", tags=["Scraping"], dependencies=[Depends(_verify_api_key)])
...
@app.post("/scraping/test-url", tags=["Scraping"], dependencies=[Depends(_verify_api_key)])
def test_url(
...
@app.get("/hotels", tags=["Hotels"], dependencies=[Depends(_verify_api_key)])
...
@app.get("/hotels/search/", tags=["Hotels"], dependencies=[Depends(_verify_api_key)])
...
@app.get("/hotels/{hotel_id}", tags=["Hotels"], dependencies=[Depends(_verify_api_key)])
...
@app.get("/export/csv", tags=["Export"], dependencies=[Depends(_verify_api_key)])
...
@app.get("/export/json", tags=["Export"], dependencies=[Depends(_verify_api_key)])
