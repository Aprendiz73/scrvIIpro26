...
def _normalize_booking_url(url: str) -> str:
    ...
    [FIX BUG-10] Esta función ahora:
    1. Emite DeprecationWarning explícito para alertar al desarrollador.
    2. Lanza ValueError si la URL no es válida, en lugar de retornar silenciosamente
       la URL original inválida (comportamiento anterior que podía insertar URLs
       corruptas en la base de datos sin ninguna indicación de error).

    ANTES: retornaba url.strip() si la validación fallaba → URL inválida en BD.
    AHORA: lanza ValueError con el motivo de rechazo → caller debe manejar el error.
    """
    warnings.warn(
        "_normalize_booking_url() está deprecada desde v3.0. "
        "Usar validate_and_normalize_booking_url() para obtener URLPipelineResult "
        "con is_valid, canonical_url y rejection_reason.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    if not result.is_valid:
        raise ValueError(
            f"URL inválida: {result.rejection_reason} (original: '{result.original_url[:80]}')"
        )
...
async def lifespan(app: FastAPI):
    ...
    print("\n" + "=" * 60)
    print("  BookingScraper Pro v4.0 - Iniciando")
    print("=" * 60)
    ...
    # [FIX BUG-06] create_directories() movida aquí desde config.py (nivel de módulo).
    # Un PermissionError en import bloqueaba todo el arranque. Ahora es controlado.
    try:
        ...
        logger.warning(f"⚠️ No se pudieron crear directorios de datos: {_dir_err}. "
                       "Las descargas de imágenes fallarán si el path no existe.")
    except Exception as _dir_err:
        logger.warning(f"⚠️ create_directories() error: {_dir_err}")

    # [FIX BUG-11] Solo activar asyncio dispatcher si Celery beat NO está configurado.
    # PROBLEMA ANTERIOR: ambos activos simultáneamente procesaban el doble de URLs.
    # Celery beat (celery_app.py) y este loop son mutuamente excluyentes.
    if not settings.USE_CELERY_DISPATCHER:
        _dispatch_task = asyncio.create_task(_auto_dispatch_loop())
        logger.info("🤖 Auto-dispatcher asyncio activado (USE_CELERY_DISPATCHER=False)")
    else:
        logger.info("🤖 Dispatch delegado a Celery beat (USE_CELERY_DISPATCHER=True)")
    ...
    try:
        ...
        # [FIX BUG-04] Solo resetear 'failed' con reintentos disponibles.
        # ANTES: reseteaba TODOS los failed incluyendo fallos permanentes →
        # bucle infinito en cada reinicio del servidor.
        # AHORA: solo URLs donde retry_count < max_retries (errores transitorios).
        # URLs con retry_count >= max_retries permanecen en 'failed' definitivo.
        r2 = _db.execute(text(
            "UPDATE url_queue SET status='pending', updated_at=NOW() "
        ...
        if r1.rowcount or r2.rowcount:
            logger.info(
                f"♻️  Reset al arrancar: {r1.rowcount} processing "
                f"+ {r2.rowcount} failed transitorios → pending"
            )
    except Exception as _e:
        logger.warning(f"⚠️ Reset al arrancar falló: {_e}")
...
app.add_middleware(
    CORSMiddleware,
    # [FIX BUG-01] allow_origins=['*'] + allow_credentials=True viola la especificación
    # W3C CORS: un servidor no puede enviar Access-Control-Allow-Origin: * cuando la
    # petición incluye credenciales. Chrome/Firefox/Edge rechazan la respuesta.
    # SOLUCIÓN: lista explícita de orígenes desde variable de entorno CORS_ORIGINS.
    # Para desarrollo: CORS_ORIGINS=http://localhost:3000,http://localhost:8080
    # Para producción: CORS_ORIGINS=https://mi-panel.empresa.com
...
def root():
    ...
    return {
        # [FIX BUG-05] "version": "3.0.0" estaba hardcodeado (incorrecto, es v4.0).
        # SOLUCIÓN: usar settings.APP_VERSION como fuente única de verdad.
