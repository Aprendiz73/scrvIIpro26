...
async def load_urls_csv(
    ...
    **CSV formats accepted:**
    - **Single column, no header:** one URL per line
    - **Column named** `url`, `URL`, `urls` or `URLs`: header detected automatically
    - **Multi-column:** first column whose header matches is used
    ...
    **Example CSV:**
    ```
    url
    https://www.booking.com/hotel/es/melia-fuerteventura.es.html
    https://www.booking.com/hotel/es/riu-palace-tres-islas.es.html
    ```
    """
    ...
    for u in raw_urls:
        if _url_has_lang_suffix(u):
            normalized_raw.append(_normalize_booking_url(u))
            normalized_count += 1
        else:
            normalized_raw.append(u)
